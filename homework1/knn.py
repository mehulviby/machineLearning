...
def compute_distances(Xtrain, X):
	...
	dists = [];
	for index, eachX in enumerate(X):
		dists.append(np.sqrt(np.sum((eachX - Xtrain)**2, axis=1)));
...
def predict_labels(k, ytrain, dists):
	...
	#####################################################
	ypredSort = []
	ypredSort = [np.argsort(dists[index])[:k] for index in range(len(dists))]
	ypredSort = [ytrain[ypredSort[index]] for index in range(len(ypredSort))]
	ypredSort = [list(np.bincount(ypredSort[i])) for i in range(len(ypredSort))]
	ypredSort = [each.index(max(each)) for each in ypredSort]
	return ypredSort
...
def compute_accuracy(y, ypred):
	...
	#				 YOUR CODE HERE					#
	#####################################################
	newVal = [y[index] for index in range(len(y)) if y[index] == ypred[index]]
	acc = len(newVal) / len(y)
	return acc 
...
def find_best_k(K, ytrain, dists, yval):
	...
	acc = []
	for index, eachK in enumerate(K):
		ypred = predict_labels(eachK, ytrain, dists);
		acc.append(compute_accuracy(yval, ypred));
	validation_accuracy = max(acc)
	best_k = acc.index(validation_accuracy)
	return K[best_k], acc
...
def data_processing(data):
	...
	# indexes = list(range(4800))
	# Xtrain = np.delete(Xtrain, indexes, axis=0)
	# ytrain = np.delete(ytrain, indexes, axis=0)
