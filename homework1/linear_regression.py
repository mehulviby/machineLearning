...
def regularized_linear_regression(X, y, lambd):
  ...
  #				 YOUR CODE HERE					#
  #####################################################	
  totalWeights = X.shape[1];
  lambdaMat = np.eye(totalWeights, dtype=int) * lambd;
  # lambdaMat[0][0] = 0
  xT = X.T;
  return np.linalg.inv(xT.dot(X) + lambdaMat).dot(xT).dot(y);
...
def tune_lambda(Xtrain, ytrain, Xval, yval, lambds):
  ...
  # error_train = [];
  error_val = [];
  for index, eachLambda in enumerate(lambds):
    error_val.append(test_error(regularized_linear_regression(Xtrain, ytrain, eachLambda), Xval, yval));
  min_index = error_val.index(min(error_val))
  return lambds[min_index]
###### Q4.4 ######
def test_error(w, X, y):
  """
    ...
    """
  sumSquared = y - X.dot(w)
  sumSquared = np.sum(sumSquared**2);
  totalWeights = X.shape[0];
  return sumSquared/totalWeights
