...
def logistic_train_ovr(Xtrain, ytrain, w, b, step_size, max_iterations):
    ...
    for i in range(max_iterations):
        w = (w - (step_size * Xtrain.T.dot(sigmoid(Xtrain.dot(w)) - ytrain)) / length)
    b_l = w[0]
    w_l = numpy.delete(w, 0)
    return w_l, b_l
...
def logistic_test_ovr(Xtest, w_l, b_l):
    ...
    """
    # you need to fill in your solution here
    w_l = numpy.insert(w_l, 0, b_l);
    Xtest = numpy.insert(Xtest, 0, 1, axis = 1);
    test_pred = sigmoid(Xtest.dot(w_l));
    return test_pred
...
def logistic_mul_train(Xtrain, ytrain, w, b, step_size, max_iterations):
    ...
    # you need to fill in your solution here
    w = numpy.insert(w, 0, b, axis = 1);
    Xtrain = numpy.insert(Xtrain, 0, 1, axis = 1);
    length = len(Xtrain)
    for i in range(max_iterations):
        w = w - (step_size * (softmax(Xtrain.dot(w.T)) - ytrain).T.dot(Xtrain) / length)
    b_l = w[:,0]
    w_l = numpy.delete(w, 0, axis=1)
    return w_l, b_l
...
#     :return:
#     """
#     sumExp = numpy.exp(inputs)
#     sumExp = sumExp / numpy.sum(sumExp, axis = 1)[:, None]
#     print (sumExp.shape)
#     return sumExp
...
def softmax(inputs):
    ...
    :return:
    """
    inputs = inputs - numpy.max(inputs, axis = 1)[:, None]
    sumExp = numpy.exp(inputs)
    return sumExp / numpy.sum(sumExp, axis = 1)[:, None]
...
def logistic_mul_test(Xtest, w_l, b_l):
    ...
    """
    # you need to fill in your solution here
    w_l = numpy.insert(w_l, 0, b_l, axis = 1);
    Xtest = numpy.insert(Xtest, 0, 1, axis = 1);
    test_pred = softmax(Xtest.dot(w_l.T));
    return test_pred
