...
def nb_train_smoothing(X, Y, vocab_size, num_classes, alpha):
    ...
    # Q8.2 Edit here
    ###################################################
    class_prob[0][0] = Y.count(0) / len(Y)
    class_prob[1][0] = Y.count(1) / len(Y)
    for idx, doc in enumerate(X):
        for val in doc:
            class_word_prob[Y[idx]][val[0]] = class_word_prob[Y[idx]][val[0]] + val[1]

    class_word_prob = class_word_prob + alpha
    class_word_prob = class_word_prob/class_word_prob.sum(axis=1)[:,None]
    class_prob = np.log(class_prob)
...
def nb_predict(X, class_prob, class_word_prob):
    ...
    ###################################################
    for idx, doc in enumerate(X):
        prob0 = class_prob[0][0]
        prob1 = class_prob[1][0]
        for val in doc:
            if class_word_prob[0][val[0]] ==0 and class_word_prob[1][val[0]] == 0:
                continue
            prob0 = val[1] * (-1e50 if class_word_prob[0][val[0]] == 0 else np.log(class_word_prob[0][val[0]])) + prob0
            prob1 = val[1] * (-1e50 if class_word_prob[1][val[0]] == 0 else np.log(class_word_prob[1][val[0]])) + prob1
        Ypred.append(0 if prob0 > prob1 else 1)
    return Ypred
