...
def pca(X = np.array([]), initial_dims = 50):
    ...
    eig_vals, eig_vecs = np.linalg.eig(X_mean.T.dot(X_mean))
    eig_pairs = [(np.abs(eig_vals[i]), eig_vecs[:,i]) for i in range(len(eig_vals))]
    eig_pairs.sort(key=lambda x: x[0], reverse=True)
    final = np.array([eig_pairs[i][1] for i in range(0,initial_dims)])
    Y = X_mean.dot(final.T)
    return Y
...
def compute_Q(Y):
    ...
    Y_sim = 1 / (1 + np.add(sum_Y, np.add(-2 * Y.dot(Y.T), sum_Y).T))
...
def compute_gradient(P, Q, Y_sim, Y, dY, no_dims):
    ...
    dY = 4 * np.subtract(np.diag(np.sum(L, 0)), L).dot(Y)
    return dY
